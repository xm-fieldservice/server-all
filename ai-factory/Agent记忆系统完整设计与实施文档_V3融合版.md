...
CREATE INDEX idx_qa_user ON qa_query_index(user_id);
```
...
PostgreSQL的行级安全（RLS）为多租户数据隔离提供了数据库层面的双重保障：

1. **设置上下文变量**:
```python
# 在每个请求开始时设置
await conn.execute("SET app.current_user_id = 'user_123'")
await conn.execute("SET app.current_agent_type = 'recruiting'")
```

2. **自动隔离效果**:
```sql
-- 以下查询自动过滤，只返回user_123的数据
SELECT * FROM entries;
-- 等价于：SELECT * FROM entries WHERE user_id = 'user_123'
```

3. **双重保障机制**:
- **应用层**: contextvars + 显式过滤（代码层面）
- **数据库层**: RLS策略强制隔离（数据库层面）
- 即使应用层代码有bug，数据库层仍能保证数据隔离

4. **性能提示**:
- RLS策略会增加约5-10%的查询开销
- 已在user_id和agent_type字段上创建复合索引，降低性能影响
...
| 隔离层 | 字段 | 作用 | 示例 |
|-------|------|------|------|
| **第一层** | agent_type | 区分Agent类型 | recruiting / customer_service / document |
| **第二层** | user_id | 区分用户（多租户） | user_123 / user_456 |
