...
- 2025-12-24：在服务器环境中使用的版本，从原始《面向 AI 工厂”的正式对接说明文档》复制，并计划在后续补充“调用路径与访问方式（写库接口）”一节，将 beibei 项目写库方式与 AI 工厂 HTTP 接口对齐。
...
> （服务器版说明）本节在服务器环境中可进一步补充：通过 `AI_FACTORY_API_BASE` 下的 HTTP 接口（例如 `POST {AI_FACTORY_API_BASE}/entries/ingest`）完成 beibei 数据写库，由 AI 工厂侧负责入库与后续解析。本文件当前版本保留为结构与字段设计说明，具体 HTTP 协议以《AI工厂-对外接口与集成规范-服务器版》为准。
