...
try:
    ...
    load_dotenv('/root/ai-factory/.env', override=True)
except Exception:
    pass
...
WECOM_NOTE_AGENT_SECRET = os.getenv("WECOM_NOTE_AGENT_SECRET", "")
WECOM_QA_AGENT_ID = os.getenv("WECOM_QA_AGENT_ID", "")
WECOM_QA_AGENT_SECRET = os.getenv("WECOM_QA_AGENT_SECRET", "")
...
import handlers  # 触发注册，确保所有处理器自动注册
import langgraph_mvp.finance_bot  # 财务 Bot
import langgraph_mvp.internal_affairs_bot  # 内务助手 Bot
...
async def wecom_callback_post(request: Request) -> PlainTextResponse:
    ...
    if agent_id_raw:
        ...
        print(f"[wecom-callback] agent_id={agent_id_raw}, handler={handler}")
        if handler:
    ...
    url = AI_FACTORY_BASE_URL.rstrip("/") + "/api/entries/ingest"
    try:
...
class DebugWecomSendTestNote(BaseModel):
    touser: str
    content: str


def _get_wecom_access_token_note() -> Optional[str]:
    """获取企业微信 NOTE应用的 access_token。"""
    if not WECOM_CORP_ID or not WECOM_NOTE_AGENT_SECRET:
        print("[note-send] Missing WECOM_CORP_ID or WECOM_NOTE_AGENT_SECRET")
        return None

    token_url = (
        "https://qyapi.weixin.qq.com/cgi-bin/gettoken"
        f"?corpid={WECOM_CORP_ID}&corpsecret={WECOM_NOTE_AGENT_SECRET}"
    )
    try:
        token_resp = requests.get(token_url, timeout=5)
        token_resp.raise_for_status()
        ...
        print(f"[note-send] ERROR getting access_token: {exc!r}")
        return None

    if token_data.get("errcode") != 0:
        print(f"[note-send] gettoken error: {token_data}")
        return None

    access_token = token_data.get("access_token", "")
    if not access_token:
        print("[note-send] no access_token returned")
        return None

    return access_token


def _send_wecom_message_note(userid: str, content: str) -> Dict[str, Any]:
    """通过企业微信 NOTE应用发送文本给指定用户。"""
    access_token = _get_wecom_access_token_note()
    if not access_token:
    ...
    send_body = {
        "touser": userid,
        "msgtype": "text",
        "agentid": int(WECOM_NOTE_AGENT_ID) if WECOM_NOTE_AGENT_ID.isdigit() else WECOM_NOTE_AGENT_ID,
    ...
    try:
        ...
        send_resp.raise_for_status()
        send_data = send_resp.json()
        print(f"[note-send] send result: {send_data}")
        return {"success": send_data.get("errcode") == 0, "result": send_data}
    except Exception as exc:
        print(f"[note-send] ERROR sending message: {exc!r}")
        return {"success": False, "error": str(exc)}


@app.post("/debug/wecom_send_note")
async def debug_wecom_send_note(payload: DebugWecomSendTestNote) -> Dict[str, Any]:
    """测试发送消息到企业微信（使用 NOTE AgentID 1000023）"""
    if not WECOM_NOTE_AGENT_ID:
        raise HTTPException(status_code=400, detail="WECOM_NOTE_AGENT_ID not configured")

    result = _send_wecom_message_note(payload.touser, payload.content)
    return result


class DebugWecomFinanceTestNote(BaseModel):
    """测试财务 Bot"""
    touser: str
    content: str


@app.post("/debug/wecom_send_finance")
async def debug_wecom_send_finance(payload: DebugWecomFinanceTestNote) -> Dict[str, Any]:
    """测试财务 Bot 并发送回复到企业微信"""
    if not WECOM_FINANCE_AGENT_ID:
        raise HTTPException(status_code=400, detail="WECOM_FINANCE_AGENT_ID not configured")
    
    from langgraph_mvp.finance_bot import handle_finance_bot
